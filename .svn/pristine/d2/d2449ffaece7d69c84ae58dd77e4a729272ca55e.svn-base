  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (ruleForm.message.length < 20) {
        ElMessage.error(
          "Please Enter Inquiry message must be more 20 characters"
        );
        return false;
      }
      // 验证结束
...
const handleQuery = async () => {
  await getLatestProduct(
    Object.assign({ keyword: companyInfo.companyCateGroup.catalog_id })
  ).then((res) => {
        ...
        >
          ...
          <div class="sr-layout-content contact-supplier">
            ...
            >
              ...
              <el-button type="primary" @click="submitForm(ruleFormRef)" style="margin-left:35%;">
                Send
              </el-button>
            </el-form>
          </div>
