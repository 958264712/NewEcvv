...
// 定义子组件向父组件传值/事件
const emit = defineEmits(['onMore','onifMore']);
            ...
            <div class="items" v-for="(item,index ) in props.leftAside.list" :key="index">
                ...
                <template v-if="item.More">
                    <ul class="items-content">
                        <li v-for="(ite,ind ) in item.list" :key="ind"><a :href="ite.href" rel="nofollow">{{ ite.text }}</a>
                        </li>
                        <template v-if="item.ifMore">
                            <li class="hidden-part" v-for="(ite,ind ) in item.moreList" :key="ind"><a :href="ite.href" rel="nofollow">{{
                                ite.text }}</a></li>
                        </template>
                    </ul>
                    <!-- more展开更多，less关闭展开  -->
                    <template v-if="item.ifMore">
                        <div class="m" @click="onifMore(item.ifMore,index)"> <span class="view-more" id="view_more" title="Click to view less">Less..</span>
                        </div>
                    </template>
                    <template v-else>
                        <div class="m" @click="onifMore(item.ifMore,index)"><span class="view-more" id="view_more" title="Click to view more">More..</span>
                        </div>
                    </template>
                </template>
            </div>
